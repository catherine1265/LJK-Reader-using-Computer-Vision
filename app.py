...
# ══════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="page-subtitle">Train SVM model untuk pengenalan karakter tulisan tangan (Nama, NIM, Tanggal).</div>', unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    <div style="margin-bottom:1rem">
      ...
      <div style="font-size:0.82rem;color:#7288AE">Latih model SVM untuk OCR menggunakan data EMNIST.</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    try:
        ...
        model_loaded = False
        st.warning("⚠️ `handwriting_ocr.py` belum tersedia.")
        ...
        with col_train:
            ...
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:1rem;
              ...
              <div>
                ...
                <div style="font-size:0.65rem;color:#4B5694;letter-spacing:0.1em;text-transform:uppercase;margin-top:2px">SVM + HOG Features</div>
              </div>
            </div>
            """, unsafe_allow_html=True)

            if st.button("🔄  Train OCR Model"):
                with st.spinner("Training SVM pada EMNIST... (ini butuh beberapa menit)"):
                    try:
            ...
            if 'ocr_accuracy' in st.session_state:
                st.markdown(f"""
        ...
        with col_pred:
            ...
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:1rem;
              ...
              <div style="width:3px;height:22px;background:linear-gradient(180deg,#EAE0CF,#7288AE);
                border-radius:2px;flex-shrink:0"></div>
              <div>
                <div style="font-family:'DM Serif Display',serif;font-size:1rem;color:#EAE0CF">Test Prediction</div>
                <div style="font-size:0.65rem;color:#4B5694;letter-spacing:0.1em;text-transform:uppercase;margin-top:2px">Upload gambar digit</div>
              </div>
            </div>
            """, unsafe_allow_html=True)

            uploaded_digit = st.file_uploader(
                "Upload gambar digit (JPG/PNG, 28x28 px ideal)",
                type=["jpg", "jpeg", "png"],
                label_visibility="collapsed"
            )

            if uploaded_digit and 'ocr_model' in st.session_state:
                from PIL import Image
                img = Image.open(uploaded_digit).convert('L')
                st.image(img, caption="Input Digit", width=100)

                try:
                    # Import untuk prediction
                    import cv2
                    import numpy as np
                    ...
                    features = hog(img_arr, orientations=9, pixels_per_cell=(4,4),
                                   cells_per_block=(2,2), visualize=False)
                    features = features.reshape(1, -1)

                    pred = st.session_state.ocr_model.predict(features)[0]
                    pred_proba = st.session_state.ocr_model.decision_function(features)[0]
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.info("💡 Untuk training penuh, pastikan `handwriting_ocr.py` sudah mengimplementasikan `load_or_train()` dengan EMNIST dataset.")
...
# ══════════════════════════════════════════════════════════════
    ...
    c1, c2, _ = st.columns([1, 1, 4])
    with c1:
        if st.button("← Handwriting"): st.session_state.step = 'handwriting'; st.rerun()
