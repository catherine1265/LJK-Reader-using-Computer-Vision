...
from config import (
    CHAR_H, CHAR_W, ALL_CHARS, MODEL_PATH, SAVE_DIR,
...
def preprocess(roi_bgr):
    ...
    if p_hi > p_lo:
        gray = np.clip((gray.astype(np.float32) - p_lo) / (p_hi - p_lo) * 255, 0, 255).astype(np.uint8)
...
def segment_fixed(binary, n_chars):
    ...
    for i in range(n_chars):
        ...
        crop = cv2.resize(crop, (CHAR_W, CHAR_H), interpolation=cv2.INTER_AREA)
        chars.append(crop)
